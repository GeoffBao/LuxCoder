...
- **Local-first data**: conversations, workspaces, attachments, settings, and Skills are stored under `~/.luxagents/` as JSON / JSONL files, without a local database.
...
Agent mode is powered by Claude Agent SDK, so it currently requires an Anthropic or Anthropic-compatible channel. Chat mode uses Provider Adapters from `@luxagents/core` to support different protocols.
...
| --- | --- | --- |
| `@luxagents/electron` | `0.9.12` | Electron desktop app |
| `@luxagents/shared` | `0.1.17` | shared types, IPC constants, config, utilities |
| `@luxagents/core` | `0.2.9` | Provider Adapters, SSE, Shiki highlighting |
| `@luxagents/ui` | `0.1.3` | shared React UI components |
