...
将 `craft-agents-max/260705-agent` 中的 Projects、Task/Conductor、Kanban 和 Teambition-Kanban 集成完整迁移到 Yoda。
...
- 功能交互与信息结构跟随权威 Craft 实现；视觉适配 Yoda 现有主题、Jotai、Radix/Tailwind 组件和三段式 App Shell。
...
迁移 atoms、类型、Board、Column、Tile、子任务、状态徽章、编辑器、Project list/detail、filter、list toggle 和 TaskChatPreview；接入 Yoda App Shell 与主题。
