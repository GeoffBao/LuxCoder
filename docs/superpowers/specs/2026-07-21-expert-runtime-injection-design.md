...
| 单元 | 位置 | 职责 |
|------|------|------|
| `resolveExpertId` | `@yoda/shared/experts` | 任务 id 优先，否则项目 default，否则 `null` |
...
5. `@yoda/shared` / `@yoda/electron` patch +1；`TaskDefaultsSchema.expertId` 注释改为「Kanban TaskRunner 已注入」  
