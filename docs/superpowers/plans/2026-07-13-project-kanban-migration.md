...
**Goal:** 将 `craft-agents-max/260705-agent` 的 Projects、Task/Conductor、Kanban 与 Teambition-Kanban 能力迁移为 Yoda 中唯一的 Work 实现，并通过冷启动、类型检查、测试和构建验证。
...
- Create: `apps/electron/src/renderer/components/app-shell/kanban/` — 从 Craft 迁移并适配 Yoda 主题的 Board、Column、Tile、TaskEditor、Preview、Filter、子任务组件。
...
- [ ] **Step 2: Add the contract/storage test directories and failing assertions**
  ...
  Add tests that import only renderer-safe types from `@yoda/shared/projects` and `@yoda/shared/tasks`, assert a duplicate node is invalid, and assert a minimal one-node spec round-trips through YAML. The tests must not import `storage.ts` through the package root.
...
- [ ] **Step 4: Run tests and typecheck**
  ...
  ```bash
  bun test packages/shared/src/projects/__tests__/storage.test.ts
  bun run --filter='@yoda/shared' typecheck
  ```
...
- [ ] **Step 4: Verify Shared exports do not pull Node storage into Vite**
  ...
  ```bash
  bun run --filter='@yoda/shared' typecheck
  bun run --filter='@yoda/electron' build:renderer
  ```
...
- [ ] **Step 4: Wire handlers through `ipc.ts` and smoke the commands**
  ...
  ```bash
  bun run --filter='@yoda/electron' typecheck
  bun run --filter='@yoda/electron' build:main
  ```
...
### Task 8: Migrate the complete Kanban and Project UI into Yoda App Shell
...
**Interfaces:**
- Consumes: `KanbanViewModel`, atoms, Preload commands, existing Yoda theme and Radix primitives.
...
- [ ] **Step 1: Port pure UI helpers and their failing tests**
  ...
  Port the Craft helper behavior for column grouping, status labels, subtask merge, node state pill, and task form validation. Keep labels and logs Chinese where the codebase already uses Chinese copy; use Yoda CSS variables rather than Craft-specific colors.
...
- [ ] **Step 1: Start the split development stack**
  ...
  ```bash
  bun run --filter='@yoda/electron' dev:vite -- --host 127.0.0.1 --port 5173
  bun run --filter='@yoda/electron' build:main
  bun run --filter='@yoda/electron' build:preload
  ```
