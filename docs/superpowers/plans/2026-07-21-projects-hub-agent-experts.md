...
**Architecture:** 扩展 `activeViewAtom`（`projects` | `agent-experts`）覆盖主区；看板仍走 `codeMainView='work'`。项目列表从侧栏迁到 `ProjectsHubView`。专家包落在 `~/.luxcodex[-dev]/experts/{slug}/`，经主进程 IPC 读写。Skills 仅以 slug 引用写入 `expert.json`。

**Tech Stack:** React 18、Jotai、Bun test、TypeScript、Electron IPC、`@luxcodex/shared` ProjectConfig、Tailwind / 现有 SidebarModule + AgentSkillsView 视觉模式。
