...
**Architecture:** `@yoda/shared/experts` 提供纯函数（resolve / merge / format）；`TaskRunnerDeps` 注入 `getExpert` 与项目 default 查找；`dispatch` 拼 `skillsPreamble(merged) + expertPreamble + buildPrompt`。不做 systemPrompt / Code 会话 / mcpIds。
...
- `@yoda/shared`: `0.1.13` → `0.1.14`
- `@yoda/electron`: `0.1.31` → `0.1.32`
