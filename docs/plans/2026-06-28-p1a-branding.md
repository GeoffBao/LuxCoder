...
**Goal:** 把 Proma 视觉/系统层面替换为 Yoda：品牌字符串、数据目录、Electron 元数据、localStorage 键名、三模式 Tab，不改任何核心功能逻辑，不改 `@proma/*` 包名（留给 P1b）。

**Architecture:** 逐文件替换，无结构变化。新增 `migration-service.ts` 在首次启动时把 `~/.yoda/` 数据迁移到 `~/.yoda/`（非破坏性，保留原目录）。ModeSwitcher 扩展为三按钮滑块（Chat / Code / Work），Work 为占位 UI。
...
| Modify | `apps/electron/src/renderer/components/settings/FeishuSettings.tsx` | UI 文案中的 Proma → Yoda |
| Modify | `apps/electron/src/renderer/components/settings/WeChatSettings.tsx` | UI 文案中的 Proma → Yoda |
...
| Rename | `apps/electron/default-skills/proma-coach/` → `yoda-coach/` | 目录 + SKILL.md 内容 |
...
/**
 * Shared configuration for Yoda
 */

export const APP_NAME = 'Yoda'
```
...
git commit -m "brand: rename APP_NAME and user-agent constants to Yoda"
```
...
mac 和 win 的 fileAssociations 把 `.yoda-backup` / `.yoda-share` 改为 `.yoda-backup` / `.yoda-share`：

```yaml
# mac section
fileAssociations:
  - ext: yoda-backup
    name: Yoda Personal Backup
    role: Editor
  - ext: yoda-share
    name: Yoda Share Package
    role: Editor
```
...
```yaml
extendInfo:
  NSMicrophoneUsageDescription: "Yoda 需要访问麦克风，用于将你的语音实时转写为文本。"
```
...
git commit -m "brand: update electron-builder metadata to Yoda"
```
...
app.setPath('userData', join(app.getPath('appData'), '@yoda/electron-dev'))
```
...
git commit -m "brand: update Electron main process to Yoda protocol/paths"
```
...
if (process.env.YODA_DEV === '1') {
  _configDirName = '.yoda-dev'
} else {
  try {
    const { app } = require('electron')
    _configDirName = app.isPackaged ? '.yoda' : '.yoda-dev'
  } catch {
    _configDirName = '.yoda'
  }
  const mode = _configDirName === '.yoda-dev' ? '开发模式' : '正式版本'
...
- [ ] **Step 3: 替换文件内所有注释中的 ~/.yoda/ 路径**

用 sed 批量替换注释（共约 30 处 JSDoc `@returns ~/.yoda/...`）：

```bash
sed -i '' 's|~\/\.proma\/|~\/.yoda\/|g' apps/electron/src/main/lib/config-paths.ts
```

- [ ] **Step 4: 替换 getSdkConfigDir 注释中的「Proma」**

```typescript
// 实现 Yoda 与 Claude Code CLI 的配置隔离。
```
...
git commit -m "brand: rename data directory .proma → .yoda"
```
 ...
 * 迁移服务
 *
 * 首次启动时将 ~/.yoda/ 迁移到 ~/.yoda/（非破坏性，保留原目录）。
...
git commit -m "feat: add data directory migration service (.proma → .yoda)"
```
...
export const TUTORIAL_TAB_TITLE = 'Yoda 使用教程'
```

- [ ] **Step 4: 更新 theme.ts 注释**

把注释中 `~/.yoda/settings.json` → `~/.yoda/settings.json`（如有）。
...
git commit -m "brand: rename proma-* localStorage keys to yoda-*"
```
...
约第 32 行：
```typescript
const GITHUB_RELEASES_URL = 'https://github.com/biajidong/Yoda/releases'
```
...
约第 470 行：
```tsx
href="https://github.com/biajidong/Yoda"
```

约第 475 行：
```tsx
github.com/biajidong/Yoda
```

- [ ] **Step 2: FeishuSettings - 用户可见文案中的 Proma → Yoda**

```bash
sed -i '' 's/Proma Agent/Yoda Agent/g' apps/electron/src/renderer/components/settings/FeishuSettings.tsx
sed -i '' 's/Proma 的/Yoda 的/g' apps/electron/src/renderer/components/settings/FeishuSettings.tsx
sed -i '' 's/Proma 会/Yoda 会/g' apps/electron/src/renderer/components/settings/FeishuSettings.tsx
sed -i '' 's/Proma 记录/Yoda 记录/g' apps/electron/src/renderer/components/settings/FeishuSettings.tsx
sed -i '' 's/让 Proma/让 Yoda/g' apps/electron/src/renderer/components/settings/FeishuSettings.tsx
sed -i '' 's/向 Proma/向 Yoda/g' apps/electron/src/renderer/components/settings/FeishuSettings.tsx
sed -i '' 's/ Proma / Yoda /g' apps/electron/src/renderer/components/settings/FeishuSettings.tsx
```
...
```bash
sed -i '' 's/Proma Agent/Yoda Agent/g' apps/electron/src/renderer/components/settings/WeChatSettings.tsx
sed -i '' 's/Proma 会/Yoda 会/g' apps/electron/src/renderer/components/settings/WeChatSettings.tsx
sed -i '' 's/向 Proma/向 Yoda/g' apps/electron/src/renderer/components/settings/WeChatSettings.tsx
sed -i '' 's/ Proma / Yoda /g' apps/electron/src/renderer/components/settings/WeChatSettings.tsx
```
...
git commit -m "brand: update settings UI copy Proma → Yoda"
```
...
**Files:**
- Rename: `apps/electron/default-skills/proma-coach/` → `apps/electron/default-skills/yoda-coach/`
...
```bash
mv apps/electron/default-skills/proma-coach apps/electron/default-skills/yoda-coach
```

- [ ] **Step 2: 更新 yoda-coach/SKILL.md 的 name 字段**

打开 `apps/electron/default-skills/yoda-coach/SKILL.md`，找到 frontmatter 中的：
```yaml
name: proma-coach
```
改为：
...
name: yoda-coach
```
同时把 SKILL.md 正文中所有 "Proma" 替换为 "Yoda"（注意保留专业术语）。
...
```typescript
/**
 * AppLogoSettings - Yoda 品牌 Logo 下载
 *
...
export const appModeAtom = atomWithStorage<AppMode>('yoda-app-mode', 'agent')
```
...
```bash
grep -rn "yoda" apps/electron/src/main apps/electron/src/renderer \
...
- logo 图片资源（proma-black.png 等）物理文件不替换 — 无 Yoda 设计稿，保留原图。
- 迁移服务只处理 `~/.yoda` → `~/.yoda` 一次性迁移；`~/.yoda-dev` 留给手动处理（开发者知道自己在做什么）。
