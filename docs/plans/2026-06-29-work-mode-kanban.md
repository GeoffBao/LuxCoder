...
| `docs/yoda/03-design-decisions.md` | 更新 Decision 7 MVP 状态 |
 ...
 * Work 模式任务类型定义
 *
 * 六阶段内部状态机（见 docs/yoda/03-design-decisions.md Decision 7）：
 ...
 * Work 任务持久化服务
 *
 * 存储：~/.yoda/work-tasks.json（全量 JSON，任务数 < 200 足够）
 * taskId 计数器持久化在同一文件，格式 { version, counter, tasks }
 */
import { randomUUID } from 'node:crypto'
import { readJsonFileSafe, writeJsonFileAtomic } from './safe-file'
import { getWorkTasksPath } from './config-paths'
import { generateTaskId } from '@yoda/shared/utils/work-task-status'
import type {
  WorkTask,
  CreateWorkTaskInput,
  UpdateWorkTaskInput,
...
- [ ] **Step 3: 检查 `@yoda/shared/utils/work-task-status` 导出路径是否已配置**
...
```typescript
import { generateTaskId } from '@yoda/shared'
```
...
import { WORK_TASK_IPC_CHANNELS } from '@yoda/shared'
```
...
```typescript
    workTask: {
      list: (): Promise<import('@yoda/shared').WorkTask[]> =>
        ipcRenderer.invoke(WORK_TASK_IPC_CHANNELS.LIST),
      get: (id: string): Promise<import('@yoda/shared').WorkTask | null> =>
        ipcRenderer.invoke(WORK_TASK_IPC_CHANNELS.GET, id),
      create: (input: import('@yoda/shared').CreateWorkTaskInput): Promise<import('@yoda/shared').WorkTask> =>
        ipcRenderer.invoke(WORK_TASK_IPC_CHANNELS.CREATE, input),
      update: (input: import('@yoda/shared').UpdateWorkTaskInput): Promise<import('@yoda/shared').WorkTask> =>
...
import type { WorkTask } from '@yoda/shared'
...
import { cn } from '@/lib/utils'
import type { SpecKitStep } from '@yoda/shared'
...
import { Check } from 'lucide-react'
import { cn } from '@/lib/utils'
import type { TbWritebackStep } from '@yoda/shared'
...
import type { WorkTaskType, WorkTaskPriority } from '@yoda/shared'
...
import { getKanbanColumn } from '@yoda/shared'
import { cn } from '@/lib/utils'
import type { WorkTask } from '@yoda/shared'
...
**注意**：此处 `import { getKanbanColumn } from '@yoda/shared'` 要确认 `getKanbanColumn` 已从 `packages/shared/src/utils/work-task-status.ts` re-export 到 `@yoda/shared` 主入口。在 `packages/shared/src/utils/index.ts` 中确认有 `export * from './work-task-status'`，并且 `packages/shared/src/index.ts` 包含 `export * from './utils'`（或类似路径）。
...
- Modify: `docs/yoda/03-design-decisions.md`
...
在 `docs/yoda/03-design-decisions.md` Decision 7 的 MVP 实现状态块更新为：

```markdown
**MVP 实现状态（2026-06-29，feature/work-mode-kanban 分支）：**
- ✅ `WorkTask` 独立实体（CB-xxx 编号 + 六阶段内部状态机 + 3 列看板展示）
- ✅ 本地 JSON 持久化（`~/.yoda/work-tasks.json`）
- ✅ 三列看板主视图（待办 | 进行中 | 待回写）
...
```bash
git add apps/electron/package.json docs/yoda/03-design-decisions.md
