...
LuxCodex 当前有 Chat / Agent / Scratch 三个模式。Work 模式（`cowork`）目前是空壳，需要改造为支持研发工程师日常工作流的协同看板。
...
2. 预填 Agent 输入框，包含：
   - 任务标题 / 类型 / 阶段
   - 输出路径（`.luxcodex-tasks/CB-001/requirements.md`）
...
MVP 需要创建 `work-sdd` Skill，作为 default skill 打包进 LuxCodex，用户创建工作区时自动获得。
...
告诉 Agent：
- 当任务阶段为**需求文档撰写**时：按 `req-template.md` 格式输出，保存到 `.luxcodex-tasks/{taskId}/requirements.md`
- 当任务阶段为**方案设计撰写**时：按 `design-template.md` 格式输出，保存到 `.luxcodex-tasks/{taskId}/design.md`
- 当任务阶段为**经验总结**时：总结本次任务的根因、解法、验证结果，保存到 `.luxcodex-tasks/{taskId}/experience.md`
...
- **产出物路径规范**：`.luxcodex-tasks/{taskId}/` 统一约定，Skill 和 IPC 服务均遵守此路径
