# Proma Code Map for LuxAgents

> 基于 `rg` 全库扫描整理，用于 LuxAgents 品牌改造与架构理解的快速导航。
...
- notes: 改造重点低，无 Proma 硬编码；若加入 LuxAgents logo 从 `LeftSidebar.tsx` 入手
...
- notes: `proma-coach` skill 名称需改为 `luxagents-coach`；每次修改 default-skills 内容必须同步递增 SKILL.md 的 `version` 字段
