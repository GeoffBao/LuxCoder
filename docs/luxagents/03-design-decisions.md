...
## Decision 4: 模式顺序 = Chat → Cowork → Code

**背景**：三个模式在 UI 中的排列顺序。

**决策**：Chat → Cowork → Code。与 Claude Desktop App 模式结构一致（对话 → 协同 → 执行）。
...
P4  Cowork Mode (4w) → TB API(MCP) + 六阶段看板 + 双向同步 + TaskOrchestrator
...
**决策**：spec-kit 作为 MCP tool 嵌入 Agent SDK，驱动 Cowork 的方案设计和编码阶段。
