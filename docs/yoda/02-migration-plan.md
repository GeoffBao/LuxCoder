...
└── apps/
    └── electron/
        └── src/
            ...
            │   └── components/ # 改造：Yoda 组件库
            └── preload/    # 改造：增加 Yoda IPC channels
```
...
| 阶段 | 名称 | 分支 | 目标 | 预估周期 |
|---|---|---|---|---|---|
| Phase 0 | Baseline | `yoda/bootstrap` | 跑通 Proma，建立代码地图 | 1 周 |
| Phase 1a | Branding | `yoda/branding` | 品牌字符串 + 主题 + 数据目录 + 三模式 Tab + 完整品牌替换清单 | 1 周 |
| Phase 1b | Namespace | `yoda/namespace` | `@proma/*` → `@yoda/*` 包名替换（300+ import） | 1 周 |
| Phase 2 | Chat Mode | `yoda/chat` | Chat 模式保持现有 provider（anthropic），和 Agent 共用；后续 Phase 接入 Hermes | 1 周 |
| Phase 3 | Work Mode | `yoda/cowork` | TB API(MCP) + 六阶段看板 + 双向同步 + TaskOrchestrator | 4 周 |
| Phase 4 | Code Mode | `yoda/code` | Agent 模式重命名为 Code（不改功能），ModeSwitcher 标签替换 | 0.5 周 |
| Phase 5 | Enterprise | `yoda/enterprise` | SSO、RBAC、Model Gateway、审计、Cost Dashboard | 4 周 |

> **模式顺序**：Chat → Work → Code（与 Claude Desktop App 一致）
> **架构决策**：P2 Runtime Adapters 已取消。Hermes 作为 Agent SDK 的 provider 接入，部署在企业内网服务端，用户只需安装 Yoda。
...
确认 Proma 原版可以在本地稳定运行，建立 Yoda 改造基线。
...
把 Proma 视觉上变成 Yoda，建立三模式 Tab 框架，迁移数据目录。不改核心功能，不改包名。
...
| `proma` | `yoda` |
| `~/.yoda` | `~/.yoda` |
| `proma-file://` 协议注册 | `yoda-file://` |
| `ai.proma.app` / `com.proma.app` | `com.luxshare.yoda` |
| `.yoda-backup` / `.yoda-share` 文件扩展名 | `.yoda-backup` / `.yoda-share` |
...
- [ ] 找到所有 `~/.yoda` 引用：`rg "\.proma" apps/ packages/`
- [ ] 统一改为 `~/.yoda`（修改 `config-paths.ts`）
- [ ] 在 `migration-service.ts` 实现首次启动自动迁移：`~/.yoda → ~/.yoda`
- [ ] 确认 `agent-sessions.json`、`conversations/`、`agent-workspaces/` 路径全部更新

### 任务清单

- [ ] 全局替换字符串 `Proma` / `proma`（品牌字符串，不含包名）
- [ ] 替换 `proma-file://` 协议注册 → `yoda-file://`
- [ ] 数据目录改为 `~/.yoda`（`config-paths.ts`）
- [ ] 在 `migration-service.ts` 实现 `~/.yoda → ~/.yoda` 迁移逻辑
- [ ] 新增 Yoda CSS 主题 tokens
- [ ] 顶部三模式 Tab 框架（暂时只是 UI，不切换功能）
- [ ] App icon 占位
- [ ] `shared/src/config/index.ts` 改 `APP_NAME = 'Yoda'`
- [ ] `electron-builder.yml` 改 `appId`、`productName`、文件扩展名
- [ ] 验证 `bun run dev` 仍然正常

### 验收标准

1. 启动后显示 Yoda，不再出现 Proma
2. 三模式 Tab 可见（功能暂为占位）
...
将 `@proma/*` 包名全部替换为 `@yoda/*`，统一 monorepo namespace。

### 改造范围

| 原 | 改 |
|---|---|
| `@proma/electron` | `@yoda/electron` |
| `@proma/shared` | `@yoda/shared` |
| `@proma/core` | `@yoda/core` |
| `@proma/ui` | `@yoda/ui` |
...
将 Proma 原有 Chat 改造为 Yoda Chat 模式，底层接 HermesAdapter。
...
| `apps/electron/src/main/lib/config-paths.ts` | `~/.yoda` → `~/.yoda` | Phase 1a |
| `apps/electron/src/main/lib/migration-service.ts` | 新增 proma→yoda 目录迁移 | Phase 1a |
| `apps/electron/src/main/index.ts` | 协议注册、窗口标题、菜单 | Phase 1a |
| `packages/*/package.json`（全部） | namespace `@proma/*` → `@yoda/*` | Phase 1b |
...
| `packages/ui/src/` | Yoda 主题 tokens | Phase 1a |
...
| 风险 | 影响 | 应对 |
|---|---|---|
| `~/.yoda` 路径改名影响已有数据 | 用户数据丢失 | `migration-service.ts` 首次启动自动迁移 |
...
| Jotai atoms 命名冲突 | 状态异常 | 新增 atoms 统一加 `yoda` 前缀 |
