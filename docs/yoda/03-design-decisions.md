...
**背景**：如果 Hermes 作为 provider，其他用户安装 Yoda 时是否需要额外安装 Hermes？

**决策**：不需要。Hermes 部署在企业内网服务端，Yoda 通过 Claude Agent SDK 的 hermes provider（HTTP endpoint）连接。用户只需安装 Yoda 一个应用。

```
Yoda (客户端) → Agent SDK → provider: "hermes" → Hermes Server (企业内网)
```
...
P1b Namespace   (1w) → @proma/* → @yoda/* 包名替换
...
项目模板（Git）   → .yoda/templates/ 目录，git clone 即得
个人模板（本地）  → 用户自建
```
...
| 看板状态 | 👤 个人 | 本地，TB 同步 |
| Skills / 模板 / RAG 知识 | 📁 项目 | Git repo `.yoda/` |
...
**产物归档**：`.yoda/specs/{task-id}/spec.md` + `plan.md`，同时上传 TB 附件。
