# Proma Code Map for Yoda

> 基于 `rg` 全库扫描整理，用于 Yoda 品牌改造与架构理解的快速导航。
...
- notes: 改造重点低，无 Proma 硬编码；若加入 Yoda logo 从 `LeftSidebar.tsx` 入手
...
- notes: `proma-coach` skill 名称需改为 `yoda-coach`；每次修改 default-skills 内容必须同步递增 SKILL.md 的 `version` 字段
...
- notes: MCP 配置按工作区隔离，存储于 `~/.yoda/agent-workspaces/{slug}/mcp.json`；内置 MCP 通过 registry.ts 注册
...
- path:
  - `apps/electron/src/main/lib/config-paths.ts` — **所有本地路径函数**，核心改造点：`~/.yoda/` → `~/.yoda/`（正式版）/ `~/.yoda-dev/` → `~/.yoda-dev/`（开发版）
  ...
  - `apps/electron/src/main/lib/attachment-service.ts` — 附件文件管理（`~/.yoda/attachments/`）
...
- notes: 无本地数据库，全部 JSON + JSONL 文件；改目录名后需提供从 `~/.yoda/` 迁移的脚本，否则老用户数据丢失
...
- path:
  - `apps/electron/electron-builder.yml` — `appId: com.proma.app`、`productName: Proma`、文件扩展名 `.yoda-backup` / `.yoda-share`、资源目录 `proma-logos`、麦克风权限描述、GitHub repo 名称
  - `packages/shared/src/config/index.ts` — `APP_NAME = 'Proma'`（全局常量，所有 UI 文案来源）
  - `apps/electron/src/main/lib/config-paths.ts` — 目录名 `.proma` / `.yoda-dev`
