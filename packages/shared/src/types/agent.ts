...
export type AgentEvent =
  ...
  | { type: 'permission_mode_changed'; mode: YodaPermissionMode }

// ===== Yoda 内部事件（SDK 不覆盖的场景） =====

/** Yoda 内部事件类型 */
export type YodaEvent =
  | { type: 'permission_request'; request: PermissionRequest }
  | { type: 'permission_resolved'; requestId: string; behavior: 'allow' | 'deny' }
  | { type: 'ask_user_request'; request: AskUserRequest }
  | { type: 'ask_user_resolved'; requestId: string }
...
export type AgentStreamPayload =
  ...
  | { kind: 'yoda_event'; event: YodaEvent }
 ...
 * Agent 会话轻量索引项
 *
 * 存储在 ~/.yoda/agent-sessions.json 中，
...
export interface AgentSessionMeta {
  ...
  /** 分叉来源：源会话的 Yoda 工作目录（SDK session 文件在此目录的项目空间中，首次 resume 后清除） */
 ...
 *
 * 按工作区隔离存储在 ~/.yoda/agent-workspaces/{slug}/session-groups.json。
 */
 ...
 * Agent 持久化消息
 *
 * 存储在 ~/.yoda/agent-sessions/{id}.jsonl 中。
 */
...
/** MCP 传输类型；Yoda 将 Streamable HTTP 规范化存储为 http */
...
/** Yoda 内置 MCP 摘要，不写入工作区 mcp.json */
...
export interface SkillMeta {
  ...
  name: string
  description?: string
  /** UI 分组名，用于把 Yoda 内嵌 Skills 收拢到同一组 */
  group?: string
  icon?: string
  version?: string
  enabled: boolean
...
export interface AgentSendInput {
  ...
  permissionModeOverride?: YodaPermissionMode
...
/** 当前 Yoda 支持的权限模式，值直接映射 SDK 原生 permissionMode */
export const YODA_PERMISSION_MODES = ['bypassPermissions', 'plan'] as const

export type YodaPermissionMode = typeof YODA_PERMISSION_MODES[number]

export const YODA_DEFAULT_PERMISSION_MODE: YodaPermissionMode = 'bypassPermissions'

export interface YodaPermissionModeConfig {
  /** 对应 Claude Agent SDK 的 permissionMode */
  sdkMode: YodaPermissionMode
  label: string
  description: string
}

/** Yoda 权限模式的单一配置来源 */
export const YODA_PERMISSION_MODE_CONFIG = {
  bypassPermissions: {
...
} as const satisfies Record<YodaPermissionMode, YodaPermissionModeConfig>

/** 权限模式定义顺序（用于循环切换） */
export const YODA_PERMISSION_MODE_ORDER: readonly YodaPermissionMode[] = YODA_PERMISSION_MODES

export function isYodaPermissionMode(mode: string): mode is YodaPermissionMode {
  return (YODA_PERMISSION_MODES as readonly string[]).includes(mode)
}

/** 规范化权限模式：历史 auto 或其它非法值统一回到默认完全自动模式 */
export function migratePermissionMode(mode: string): YodaPermissionMode {
  if (isYodaPermissionMode(mode)) return mode
  return YODA_DEFAULT_PERMISSION_MODE
}
