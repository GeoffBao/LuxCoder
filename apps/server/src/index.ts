...
app.get('/api/health', (c) => c.json({ ok: true, service: 'luxcoder-server' }))
...
if (import.meta.main) {
  ...
  console.log(`[LuxCoder Server] 启动中 http://localhost:${port}`)
  const server = Bun.serve({
    port,
    fetch: app.fetch,
  })
  console.log(`[LuxCoder Server] 已监听 http://localhost:${server.port}`)
}
