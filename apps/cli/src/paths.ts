 ...
 * 判断 isPackaged 来在 .luxcoder / .luxcoder-dev 间切换——CLI 没有 electron 运行时，
 * 因此这里独立实现一份等价逻辑：
 *   - 默认 ~/.luxcoder
 *   - 环境变量 LUXCODER_DEV=1（兼容 PROMA_DEV=1）→ ~/.luxcoder-dev
...
export interface PathOptions {
  /** 显式指定配置目录（绝对路径）。优先级最高。 */
  configDir?: string
  /** 使用开发目录 .luxcoder-dev（等价于 LUXCODER_DEV=1）。 */
  dev?: boolean
}
...
export function resolveConfigDir(opts: PathOptions = {}): string {
  ...
  const useDev =
    opts.dev || process.env.LUXCODER_DEV === '1' || process.env.PROMA_DEV === '1'
  return join(homedir(), useDev ? '.luxcoder-dev' : '.luxcoder')
}
