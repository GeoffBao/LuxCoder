 ...
 * 判断 isPackaged 来在 .yoda / .yoda-dev 间切换——CLI 没有 electron 运行时，
 * 因此这里独立实现一份等价逻辑：
 *   - 默认 ~/.yoda
 *   - 环境变量 YODA_DEV=1（兼容 PROMA_DEV=1）→ ~/.yoda-dev
...
export interface PathOptions {
  /** 显式指定配置目录（绝对路径）。优先级最高。 */
  configDir?: string
  /** 使用开发目录 .yoda-dev（等价于 YODA_DEV=1）。 */
  dev?: boolean
}
...
export function resolveConfigDir(opts: PathOptions = {}): string {
  ...
  const useDev =
    opts.dev || process.env.YODA_DEV === '1' || process.env.PROMA_DEV === '1'
  return join(homedir(), useDev ? '.yoda-dev' : '.yoda')
}
