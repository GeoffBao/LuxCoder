#!/usr/bin/env bun
/**
 * luxcodex — LuxCodex 命令行工具入口。
 *
 * 用法：
 *   luxcodex <command> [subcommand] [args] [--flags]
 *   luxcodex session list|info|outline|search|export ...
 *
 * 全局 flag：
 *   --json            输出机器可读 JSON
 *   --config-dir DIR  指定 LuxCodex 配置目录（默认 ~/.luxcodex，LUXCODEX_DEV=1 → ~/.luxcodex-dev）
 *   --dev             使用 ~/.luxcodex-dev
 *
...
function printHelp(): void {
  info('luxcodex — LuxCodex 会话渐进式读取 CLI\n')
  info('用法: luxcodex session <command> [args] [--flags]\n')
  info('命令:')
...
async function main(): Promise<number> {
  ...
  // 命名空间：当前只有 session，支持 `luxcodex session <cmd>` 与直接 `luxcodex <cmd>`
  let rest = argv
  ...
  try {
    ...
    if (err instanceof UsageError) {
      ...
      info(`用法: luxcodex ${command.usage}`)
      return EXIT_USAGE
    }
