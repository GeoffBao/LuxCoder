 ...
 *
 * 用法：
 *   yoda <command> [subcommand] [args] [--flags]
 *   yoda session list|info|outline|search|export ...
 *
 * 全局 flag：
 *   --json            输出机器可读 JSON
 *   --config-dir DIR  指定 Yoda 配置目录（默认 ~/.yoda，YODA_DEV=1 → ~/.yoda-dev）
 *   --dev             使用 ~/.yoda-dev
 *
...
function printHelp(): void {
  info('yoda — Yoda 会话渐进式读取 CLI\n')
  info('用法: yoda session <command> [args] [--flags]\n')
  info('命令:')
...
async function main(): Promise<number> {
  ...
  // 命名空间：当前只有 session，支持 `yoda session <cmd>` 与直接 `yoda <cmd>`
  let rest = argv
