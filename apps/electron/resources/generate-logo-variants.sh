...
# 从统一的 Yoda L 母版生成设置页与品牌下载使用的全部图标变体。
...
MARK_SVG="$LOGOS_DIR/yoda-mark.svg"
TEMP_DIR="$(mktemp -d "${TMPDIR:-/tmp}/yoda-logo-variants.XXXXXX")"
...
if [ ! -f "$MARK_SVG" ]; then
  echo "❌ Yoda mark not found: $MARK_SVG"
  exit 1
fi
...
echo "✅ Generated 14 Yoda logo variants in $LOGOS_DIR"
