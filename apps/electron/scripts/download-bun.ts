...
import type { PlatformArch, BunDownloadInfo } from '@luxagents/shared'
...
async function getBunVersion(): Promise<string> {
  ...
  const version = pkg.luxagents?.bun?.version
  if (!version) {
    throw new Error('package.json 中未配置 luxagents.bun.version')
  }
