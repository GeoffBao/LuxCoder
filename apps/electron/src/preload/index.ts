...
import type { ExpertManifest, ExpertPackage } from '@yoda/shared/experts'
...
/** TB 缺陷看板：缺陷任务视图模型（与 @yoda/shared/teambition-defect 对齐） */
...
/** Teambition MCP 识别结果（与 @yoda/shared teambition-mcp 一致） */
export type BrowserTeambitionRecognition = import('@yoda/shared').TeambitionMcpRecognition
...
export interface ElectronAPI {
  ...
  /** 获取未暂存的变更文件列表 */
  getUnstagedChanges: (dirPath: string, sessionPath?: string, workspaceFilesPath?: string, extraPaths?: string[], sessionId?: string) => Promise<import('@yoda/shared').UnstagedChangesResult>
  /** 获取单个文件的 diff */
  getFileDiff: (input: import('@yoda/shared').GetFileDiffInput) => Promise<string>
  /** 获取未追踪文件内容 */
  getUntrackedContent: (input: import('@yoda/shared').GetFileDiffInput) => Promise<string>
  /** 还原文件变更 */
  revertFile: (input: import('@yoda/shared').RevertFileInput) => Promise<void>
  /** 获取文件新旧版本内容 */
  getDiffContents: (input: import('@yoda/shared').GetFileDiffInput) => Promise<{ oldContent: string; newContent: string } | null>
  /** 列出 Git Worktree */
  listWorktrees: (repoPath: string, sessionId: string) => Promise<import('@yoda/shared').WorktreeInfo[]>
  ...
  getWorktreeChanges: (worktreePath: string, baseBranch: string, sessionId: string) => Promise<import('@yoda/shared').UnstagedChangesResult>
  /** 在独立窗口打开当前文件预览 */
  ...
  getPiReasoningCapability: (channelId: string, modelId: string) => Promise<import('@yoda/shared').ReasoningCapability | undefined>
  ...
  /** 测试 MCP 服务器连接 */
  testMcpServer: (name: string, entry: import('@yoda/shared').McpServerEntry) => Promise<{ success: boolean; message: string }>
  ...
  /** 获取默认 Skills 的 slug 列表（来自 ~/.yoda/default-skills/） */
  ...
  /** 从其他工作区批量导入多个 Skill */
  batchImportSkillsFromWorkspaces: (targetSlug: string, selections: import('@yoda/shared').BulkImportWorkspaceSelection[]) => Promise<import('@yoda/shared').BulkImportSkillsResult>

  /** 选择本地 Skill 导入源（zip 压缩包或文件夹） */
  pickLocalSkillSource: () => Promise<{ kind: 'zip' | 'folder'; path: string } | null>

  /** 从本地 zip / 文件夹导入 Skill 到目标工作区 */
  importSkillsFromLocal: (targetSlug: string, sourcePath: string) => Promise<import('@yoda/shared').BulkImportSkillsResult>
  ...
  listSkillFiles: (workspaceSlug: string, skillSlug: string) => Promise<import('@yoda/shared').SkillFileNode[]>

  /** 读取 Skill 目录下的子文件内容 */
  readSkillFile: (workspaceSlug: string, skillSlug: string, relativePath: string) => Promise<import('@yoda/shared').SkillFileContent>
  ...
  /** 读取工作区 CLAUDE.md */
  readWorkspaceClaudeMd: (workspaceSlug: string) => Promise<import('@yoda/shared').SkillFileContent>

  /** 写入工作区 CLAUDE.md */
  writeWorkspaceClaudeMd: (workspaceSlug: string, content: string) => Promise<void>

  /** 列出工作区 auto memory 文件树 */
  listWorkspaceAutoMemoryFiles: (workspaceSlug: string) => Promise<import('@yoda/shared').SkillFileNode[]>

  /** 读取工作区 auto memory 文件 */
  readWorkspaceAutoMemoryFile: (workspaceSlug: string, relativePath: string) => Promise<import('@yoda/shared').SkillFileContent>
  ...
  updateSessionPermissionMode: (sessionId: string, mode: YodaPermissionMode) => Promise<void>
  ...
  getWorktreeRepos: (workspaceSlug: string) => Promise<import('@yoda/shared').WorkspaceWorktreeRepo[]>
  /** 添加 worktree 仓库到工作区配置 */
  addWorktreeRepo: (workspaceSlug: string, repo: import('@yoda/shared').WorkspaceWorktreeRepo) => Promise<import('@yoda/shared').WorkspaceWorktreeRepo[]>
  /** 从工作区配置移除 worktree 仓库 */
  removeWorktreeRepo: (workspaceSlug: string, repoPath: string) => Promise<import('@yoda/shared').WorkspaceWorktreeRepo[]>
  ...
  /** 列出目录内容 */
  listDirectory: (dirPath: string, access?: import('@yoda/shared').FileAccessOptions) => Promise<FileEntry[]>

  /** 删除文件/目录 */
  deleteFile: (filePath: string, access?: import('@yoda/shared').FileAccessOptions) => Promise<void>

  /** 用系统默认应用打开文件 */
  openFile: (filePath: string, access?: import('@yoda/shared').FileAccessOptions) => Promise<void>

  /** 将剪贴板文本写入临时预览文件并返回绝对路径 */
  writeClipboardPreview: (filename: string, content: string) => Promise<string>

  /** 用系统默认应用打开任意文件（无工作区限制） */
  systemOpenFile: (filePath: string, appName?: string, access?: import('@yoda/shared').FileAccessOptions) => Promise<void>

  /** 扫描系统中可用的编辑器应用（仅 macOS） */
  scanEditors: () => Promise<import('@yoda/shared').EditorApp[]>

  /** 查询本机为该文件类型注册的默认打开应用（含图标 dataURL） */
  getDefaultAppForFile: (filePath: string, access?: import('@yoda/shared').FileAccessOptions) => Promise<import('@yoda/shared').DefaultAppInfo | null>

  /** 在系统文件管理器中显示文件 */
  showInFolder: (filePath: string, access?: import('@yoda/shared').FileAccessOptions) => Promise<void>
  ...
  /** 解析文件路径并读取内容（供内联预览使用） */
  resolveAndReadFile: (filePath: string, access?: import('@yoda/shared').FileAccessOptions) => Promise<{ resolvedPath: string; content: string } | null>

  /** 写入文本文件（供 Markdown 内联编辑使用） */
  writeTextFile: (filePath: string, content: string, access?: import('@yoda/shared').FileAccessOptions) => Promise<boolean>

  /** 仅解析文件路径（供 PDF/图片等用 file:// 加载） */
  resolveFilePath: (filePath: string, access?: import('@yoda/shared').FileAccessOptions) => Promise<import('@yoda/shared').ResolvedFileUrl | null>

  /** 为内联 PDF 预览生成临时 HTML 文件，返回文件路径 */
  preparePdfPreview: (filePath: string, access?: import('@yoda/shared').FileAccessOptions) => Promise<{ tmpHtmlUrl: string } | null>

  /** 为内联 HTML 预览注册文件所在目录 URL（相对路径资源自动解析） */
  prepareHtmlPreview: (filePath: string, access?: import('@yoda/shared').FileAccessOptions) => Promise<{ tmpUrl: string } | null>

  /** 读取文件为 base64（带路径校验，供内联图片预览等） */
  readBinaryBase64: (filePath: string, access?: import('@yoda/shared').FileAccessOptions, maxSize?: number) => Promise<string | null>

  /** DOCX 转 HTML（内联预览） */
  docxToHtml: (filePath: string, access?: import('@yoda/shared').FileAccessOptions) => Promise<{ resolvedPath: string; html: string } | null>

  /** XLSX/PPTX 转 HTML（内联预览） */
  officeToHtml: (filePath: string, access?: import('@yoda/shared').FileAccessOptions) => Promise<import('@yoda/shared').OfficePreviewResult | null>
  ...
  /** 重命名文件/目录 */
  renameFile: (filePath: string, newName: string, access?: import('@yoda/shared').FileAccessOptions) => Promise<void>

  /** 移动文件/目录到目标目录 */
  moveFile: (filePath: string, targetDir: string, access?: import('@yoda/shared').FileAccessOptions) => Promise<void>

  /** 列出附加目录内容 */
  listAttachedDirectory: (dirPath: string, access?: import('@yoda/shared').FileAccessOptions) => Promise<FileEntry[]>

  /** 读取附加目录文件内容为 base64（限制在已附加目录范围内） */
  readAttachedFile: (filePath: string, sessionId?: string, workspaceSlug?: string) => Promise<string>

  /** 在文件管理器中显示附加目录文件 */
  showAttachedInFolder: (filePath: string, access?: import('@yoda/shared').FileAccessOptions) => Promise<void>

  /** 重命名附加目录文件/目录（无工作区路径限制） */
  renameAttachedFile: (filePath: string, newName: string, access?: import('@yoda/shared').FileAccessOptions) => Promise<void>

  /** 移动附加目录文件/目录（无工作区路径限制） */
  moveAttachedFile: (filePath: string, targetDir: string, access?: import('@yoda/shared').FileAccessOptions) => Promise<void>
  ...
  /** 获取多 Bot 配置 */
  getFeishuMultiConfig: () => Promise<import('@yoda/shared').FeishuMultiBotConfig>
  /** 保存单个 Bot 配置 */
  saveFeishuBotConfig: (input: import('@yoda/shared').FeishuBotConfigInput) => Promise<import('@yoda/shared').FeishuBotConfig>
  ...
  /** 获取多 Bot 状态 */
  getFeishuMultiStatus: () => Promise<import('@yoda/shared').FeishuMultiBridgeState>

  // --- 扫码注册 ---

  /** 启动扫码注册流程，等待用户扫码 + 飞书确认后返回 App ID/Secret */
  registerFeishuApp: () => Promise<import('@yoda/shared').FeishuRegisterAppResult>
  /** 取消正在进行的扫码注册流程 */
  cancelFeishuRegistration: () => Promise<void>
  /** 监听二维码 URL 生成 */
  onFeishuRegisterQrcode: (callback: (payload: import('@yoda/shared').FeishuRegisterAppQRCode) => void) => () => void
  /** 监听注册流程状态变化 */
  onFeishuRegisterStatus: (callback: (payload: import('@yoda/shared').FeishuRegisterAppStatus) => void) => () => void
  ...
  /** 获取多 Bot 配置 */
  getDingTalkMultiConfig: () => Promise<import('@yoda/shared').DingTalkMultiBotConfig>
  /** 保存单个 Bot 配置 */
  saveDingTalkBotConfig: (input: import('@yoda/shared').DingTalkBotConfigInput) => Promise<import('@yoda/shared').DingTalkBotConfig>
  ...
  /** 获取多 Bot 状态 */
  getDingTalkMultiStatus: () => Promise<import('@yoda/shared').DingTalkMultiBridgeState>
  ...
  /** 打开文件选择对话框（选择 .yoda-backup 或 .yoda-share） */
...
const electronAPI: ElectronAPI = {
  ...
  getFileDiff: (input: import('@yoda/shared').GetFileDiffInput) => {
    return ipcRenderer.invoke(IPC_CHANNELS.GET_FILE_DIFF, input)
  },

  getUntrackedContent: (input: import('@yoda/shared').GetFileDiffInput) => {
    return ipcRenderer.invoke(IPC_CHANNELS.GET_UNTRACKED_CONTENT, input)
  },

  revertFile: (input: import('@yoda/shared').RevertFileInput) => {
    return ipcRenderer.invoke(IPC_CHANNELS.REVERT_FILE, input)
  },

  getDiffContents: (input: import('@yoda/shared').GetFileDiffInput) => {
  ...
  testMcpServer: (name: string, entry: import('@yoda/shared').McpServerEntry) => {
  ...
  batchImportSkillsFromWorkspaces: (targetSlug: string, selections: import('@yoda/shared').BulkImportWorkspaceSelection[]) => {
    return ipcRenderer.invoke(
  ...
  updateSessionPermissionMode: (sessionId: string, mode: YodaPermissionMode) => {
  ...
  addWorktreeRepo: (workspaceSlug: string, repo: import('@yoda/shared').WorkspaceWorktreeRepo) => {
  ...
  listDirectory: (dirPath: string, access?: import('@yoda/shared').FileAccessOptions) => {
    return ipcRenderer.invoke(AGENT_IPC_CHANNELS.LIST_DIRECTORY, dirPath, access)
  },

  deleteFile: (filePath: string, access?: import('@yoda/shared').FileAccessOptions) => {
    return ipcRenderer.invoke(AGENT_IPC_CHANNELS.DELETE_FILE, filePath, access)
  },

  openFile: (filePath: string, access?: import('@yoda/shared').FileAccessOptions) => {
  ...
  systemOpenFile: (filePath: string, appName?: string, access?: import('@yoda/shared').FileAccessOptions) => {
  ...
  getDefaultAppForFile: (filePath: string, access?: import('@yoda/shared').FileAccessOptions) => {
    return ipcRenderer.invoke(IPC_CHANNELS.GET_DEFAULT_APP_FOR_FILE, filePath, access) as Promise<import('@yoda/shared').DefaultAppInfo | null>
  },

  showInFolder: (filePath: string, access?: import('@yoda/shared').FileAccessOptions) => {
  ...
  resolveAndReadFile: (filePath: string, access?: import('@yoda/shared').FileAccessOptions) => {
    return ipcRenderer.invoke('file:resolve-and-read', filePath, access) as Promise<{ resolvedPath: string; content: string } | null>
  },

  writeTextFile: (filePath: string, content: string, access?: import('@yoda/shared').FileAccessOptions) => {
    return ipcRenderer.invoke('file:write-text', filePath, content, access) as Promise<boolean>
  },

  resolveFilePath: (filePath: string, access?: import('@yoda/shared').FileAccessOptions) => {
    return ipcRenderer.invoke('file:resolve-path', filePath, access) as Promise<import('@yoda/shared').ResolvedFileUrl | null>
  },

  preparePdfPreview: (filePath: string, access?: import('@yoda/shared').FileAccessOptions) => {
    return ipcRenderer.invoke('file:prepare-pdf-preview', filePath, access) as Promise<{ tmpHtmlUrl: string } | null>
  },

  prepareHtmlPreview: (filePath: string, access?: import('@yoda/shared').FileAccessOptions) => {
    return ipcRenderer.invoke('file:prepare-html-preview', filePath, access) as Promise<{ tmpUrl: string } | null>
  },

  readBinaryBase64: (filePath: string, access?: import('@yoda/shared').FileAccessOptions, maxSize?: number) => {
    return ipcRenderer.invoke('file:read-binary-base64', filePath, access, maxSize) as Promise<string | null>
  },

  docxToHtml: (filePath: string, access?: import('@yoda/shared').FileAccessOptions) => {
    return ipcRenderer.invoke('file:docx-to-html', filePath, access) as Promise<{ resolvedPath: string; html: string } | null>
  },

  officeToHtml: (filePath: string, access?: import('@yoda/shared').FileAccessOptions) => {
    return ipcRenderer.invoke('file:office-to-html', filePath, access) as Promise<import('@yoda/shared').OfficePreviewResult | null>
  },
  ...
  renameFile: (filePath: string, newName: string, access?: import('@yoda/shared').FileAccessOptions) => {
    return ipcRenderer.invoke(AGENT_IPC_CHANNELS.RENAME_FILE, filePath, newName, access)
  },

  moveFile: (filePath: string, targetDir: string, access?: import('@yoda/shared').FileAccessOptions) => {
    return ipcRenderer.invoke(AGENT_IPC_CHANNELS.MOVE_FILE, filePath, targetDir, access)
  },

  listAttachedDirectory: (dirPath: string, access?: import('@yoda/shared').FileAccessOptions) => {
  ...
  showAttachedInFolder: (filePath: string, access?: import('@yoda/shared').FileAccessOptions) => {
    return ipcRenderer.invoke(AGENT_IPC_CHANNELS.SHOW_ATTACHED_IN_FOLDER, filePath, access)
  },

  renameAttachedFile: (filePath: string, newName: string, access?: import('@yoda/shared').FileAccessOptions) => {
    return ipcRenderer.invoke(AGENT_IPC_CHANNELS.RENAME_ATTACHED_FILE, filePath, newName, access)
  },

  moveAttachedFile: (filePath: string, targetDir: string, access?: import('@yoda/shared').FileAccessOptions) => {
  ...
  saveFeishuBotConfig: (input: import('@yoda/shared').FeishuBotConfigInput) => {
  ...
  onFeishuRegisterQrcode: (callback: (payload: import('@yoda/shared').FeishuRegisterAppQRCode) => void) => {
    const listener = (_: unknown, payload: import('@yoda/shared').FeishuRegisterAppQRCode) => callback(payload)
  ...
  onFeishuRegisterStatus: (callback: (payload: import('@yoda/shared').FeishuRegisterAppStatus) => void) => {
    const listener = (_: unknown, payload: import('@yoda/shared').FeishuRegisterAppStatus) => callback(payload)
  ...
  saveDingTalkBotConfig: (input: import('@yoda/shared').DingTalkBotConfigInput) => {
  ...
  onQuickTaskEvent: (callback: (event: QuickTaskWindowEvent) => void) => {
    // Yoda: planning quick-task IPC stub, to be completed with quick-task window
...
const yodaWindowKind = process.argv
  .find((arg) => arg.startsWith('--yoda-window='))
  ?.slice('--yoda-window='.length)

// 将 API 暴露到渲染进程的 window 对象上
contextBridge.exposeInMainWorld('electronAPI', electronAPI)
contextBridge.exposeInMainWorld('__yodaWindowKind', yodaWindowKind)

// 扩展 Window 接口的类型定义
declare global {
  interface Window {
    electronAPI: ElectronAPI
    __yodaWindowKind?: string
  }
}
