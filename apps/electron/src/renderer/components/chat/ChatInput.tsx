 ...
 * - 监听 luxagents:clear-context 和 luxagents:focus-input 自定义事件
 * - 卡片式容器样式
 */
...
export function ChatInput({ conversationId, streaming, pendingAttachments, onSetPendingAttachments, onSend, onStop, onClearContext }: ChatInputProps): React.ReactElement {
  ...
  React.useEffect(() => {
    ...
    window.addEventListener('luxagents:clear-context', handler)
    return () => window.removeEventListener('luxagents:clear-context', handler)
  }, [onClearContext])
