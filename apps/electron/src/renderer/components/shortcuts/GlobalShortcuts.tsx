...
export function GlobalShortcuts(): null {
  ...
  useShortcut(
    'clear-context',
    useCallback(() => {
      window.dispatchEvent(new CustomEvent('luxcodex:clear-context'))
    }, []),
  )
  ...
  useShortcut(
    'focus-input',
    useCallback(() => {
      window.dispatchEvent(new CustomEvent('luxcodex:focus-input'))
    }, []),
  )
  ...
  useShortcut(
    'stop-generation',
    useCallback(() => {
      window.dispatchEvent(new CustomEvent('luxcodex:stop-generation'))
    }, []),
  )
    ...
    const cleanup = window.electronAPI.onQuickTaskOpenSession(async (data) => {
      try {
        ...
        if (data.mode === 'agent') {
          ...
          const savedAttachments: import('@luxcodex/shared').FileAttachment[] = []
  ...
  useEffect(() => {
    const cleanup = window.electronAPI.onVoiceDictationInsertText(({ text }) => {
      ...
      const insertedAtCursor = !window.dispatchEvent(new CustomEvent('luxcodex:insert-voice-dictation-text', {
        cancelable: true,
        detail: { text: trimmed },
      }))
      if (insertedAtCursor) {
