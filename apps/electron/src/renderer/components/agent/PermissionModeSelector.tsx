...
import type { YodaPermissionMode } from '@yoda/shared'
import { YODA_PERMISSION_MODE_CONFIG, YODA_PERMISSION_MODE_ORDER } from '@yoda/shared'
...
const MODE_ICONS: Record<YodaPermissionMode, React.ComponentType<{ className?: string }>> = {
...
export function PermissionModeSelector({ sessionId }: PermissionModeSelectorProps): React.ReactElement | null {
  ...
  React.useEffect(() => {
    ...
    setModeMap((prev: Map<string, YodaPermissionMode>) => {
  ...
  const cycleMode = React.useCallback(async () => {
    const currentIndex = YODA_PERMISSION_MODE_ORDER.indexOf(displayMode)
    const nextIndex = (currentIndex + 1) % YODA_PERMISSION_MODE_ORDER.length
    const nextMode = YODA_PERMISSION_MODE_ORDER[nextIndex]!
    const prevMode = mode
  ...
  const config = YODA_PERMISSION_MODE_CONFIG[displayMode]
