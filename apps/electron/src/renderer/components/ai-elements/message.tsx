  ...
  function MessageResponse({ children, className, basePath, basePaths, remarkPlugins }: MessageResponseProps): React.ReactElement {
    ...
    return (
      ...
      >
        ...
        >
          {normalizeLatexDelimiters(children.replace(/<!--LUXCODER_AUTOMATION:[\s\S]*?-->/g, '').trim())}
        </Markdown>
      </div>
    )
  },
...
export const UserMessageContent = React.memo(
  function UserMessageContent({ children, className, ...props }: UserMessageContentProps): React.ReactElement {
    ...
    return (
      <div className={cn('user-message-bubble relative inline-block min-w-[4em] max-w-full whitespace-normal rounded-[12px] bg-secondary/75 px-3.5 py-2.5', className)} {...props}>
        <div
          ref={contentRef}
          className={cn(
