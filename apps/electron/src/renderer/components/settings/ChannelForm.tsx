...
import { normalizeBaseUrl, resolveAnthropicMessagesUrl, resolveOpenAIChatCompletionsUrl, resolveOpenAIResponsesUrl } from '@yoda/core'
          ...
          <div className="px-4 py-3 space-y-2">
            ...
            {isCodexProvider ? (
              <div className="space-y-2">
                ...
                ) : <div className="text-xs text-muted-foreground">Yoda 会代理 token 请求；系统浏览器授权页仍需使用可访问 OpenAI 的网络。可改用设备码并在另一台设备完成授权。</div>}
              </div>
            ) : isXaiProvider ? (
              <div className="space-y-2">
                <Button
                ...
                ) : <div className="text-xs text-muted-foreground">Yoda 会代理设备码与 token 请求；授权页由系统浏览器打开。若浏览器无可用代理，可扫码在另一台设备完成授权。</div>}
              </div>
            ) : isClaudeOAuthProvider ? (
              <div className="space-y-2">
                <Button
          ...
          <AlertDialogHeader>
            ...
            <AlertDialogDescription asChild>
              <div className="space-y-2">
                ...
                <p>Yoda 仅作为本地 Agent 执行环境：配置、会话等本地数据均存储在你的设备上，Yoda 本身不会额外构成数据风险。</p>
              </div>
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
