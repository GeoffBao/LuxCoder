...
如果走方式 B，**记下"用户的 nmem 实际命令是 `uvx nmem-cli`"**——这个信息当前 Step 4 的 hooks 不直接用到（hooks 是调用 `~/.luxcoder/scripts/` 下的 Python 脚本，跟 nmem CLI 无关），但用户排错或自己用 nmem 时需要知道。
...
mkdir -p ~/.luxcoder/scripts ~/.luxcoder/agent-workspaces/<工作区名>/skills
cp /tmp/nowledge-community/nowledge-mem-proma-plugin/hooks/save-to-nmem.py ~/.luxcoder/scripts/
cp /tmp/nowledge-community/nowledge-mem-proma-plugin/hooks/read-working-memory.py ~/.luxcoder/scripts/
chmod +x ~/.luxcoder/scripts/save-to-nmem.py ~/.luxcoder/scripts/read-working-memory.py
cp -R /tmp/nowledge-community/nowledge-mem-proma-plugin/skills/{read-working-memory,search-memory,distill-memory,save-thread,status} ~/.luxcoder/agent-workspaces/<工作区名>/skills/
```

## Step 3：配置目标工作区的 MCP
在 `~/.luxcoder/agent-workspaces/<工作区名>/mcp.json` 中创建或编辑（顶层 key 必须是 `servers`）：
...
## Step 4：配置 Hooks
编辑 `~/.luxcoder/sdk-config/.claude/settings.json`，添加以下 hooks。如果文件已有内容则合并，不要覆盖。
...
export PATH="$HOME/.local/bin:$PATH"
nmem status
python3 ~/.luxcoder/scripts/read-working-memory.py
grep 'nowledge-mem:start' ~/.luxcoder/agent-workspaces/<工作区名>/CLAUDE.md && echo "Block 已注入" || echo "Block 缺失"

# 检查 5 个 skill 是否都已安装到目标工作区
for s in read-working-memory search-memory distill-memory save-thread status; do
  if [ -d ~/.luxcoder/agent-workspaces/<工作区名>/skills/$s ]; then
    echo "✅ skill: $s"
  else
    echo "❌ skill: $s 缺失"
  fi
...
重启后在新会话里验证：
1. `cat ~/.luxcoder/logs/nm-hooks.log` — 确认有新的 SessionStart 记录
