...
import { highlightCode, highlightToTokens, getDisplayName } from '@yoda/core'
import { MermaidBlock } from '@yoda/ui'
import type { HighlightTokensResult } from '@yoda/core'
import type { FileAccessOptions } from '@yoda/shared'
...
function buildCodeBlockRenderModeDecorations(doc: ProseMirrorNode, editable: boolean): DecorationSet {
  ...
  doc.descendants((node, pos) => {
    ...
    decorations.push(Decoration.node(pos, pos + node.nodeSize, {
      'data-yoda-render-mode': editable ? 'editing' : 'preview',
    }))
    return false
  })
  return DecorationSet.create(doc, decorations)
...
function isExternalUrl(src: string): boolean {
  return /^(?:https?:|data:|blob:|file:|yoda-file:)/i.test(src)
}
...
function resolveMediaSrc(src: string, fileAccessRef: FileAccessRefOrNull, apply: (src: string) => void): () => void {
  // 外链 / data-URL / blob / 已授权 yoda-file 协议：直接 apply，不走 IPC
...
function createMarkdownImageView(initialNode: ProseMirrorNode, fileAccessRef: FileAccessRefOrNull, editor: any, getPos: (() => number | undefined) | boolean) {
  ...
  // 用 ResizableNodeView 实现缩放（仅可编辑模式）
...
function createShikiCodeBlockView(initialNode: ProseMirrorNode, view: EditorView) {
  ...
  dom.dataset.yodaCodeBlock = 'true'

  // 头部栏：语言标签 + 复制按钮
  const header = document.createElement('div')
  header.contentEditable = 'false'
  setClass(header, 'yoda-code-header flex h-8 items-center justify-between border-b border-border/30 px-3 text-xs text-muted-foreground')
  ...
  setClass(body, 'yoda-code-source-body markdown-code-block-body overflow-x-auto')
  ...
  setClass(mermaidHost, 'yoda-mermaid-preview hidden')
  ...
  const render = (node: ProseMirrorNode) => {
    ...
    dom.classList.toggle('yoda-code-block--mermaid', shouldRenderMermaid)
