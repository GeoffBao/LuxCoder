...
const MATCH_SELECTOR = 'mark[data-luxcodex-find-match]'
const SHADOW_STYLE_ID = 'luxcodex-find-highlight-style'
...
function injectShadowStyle(root: ParentNode): void {
  ...
  style.textContent = `
    ...
    ${MATCH_SELECTOR}[data-luxcodex-find-active="true"] {
...
function shouldSkipTextNode(node: Text): boolean {
  ...
  if (parent.closest('[data-luxcodex-find-ignore]')) return true
...
function markMatchesInTextNode(node: Text, matcher: RegExp): HTMLElement[] {
  ...
  for (const item of matches) {
    ...
    mark.dataset.luxcodexFindMatch = 'true'
...
function applyHighlights(container: HTMLElement, query: string, options: FindOptions): HTMLElement[] {
  ...
  marks.forEach((mark, index) => {
    mark.dataset.luxcodexFindIndex = String(index)
  })
  return marks
}
...
function setActiveMatch(marks: HTMLElement[], activeIndex: number): void {
  marks.forEach((mark, index) => {
    if (index === activeIndex) {
      mark.dataset.luxcodexFindActive = 'true'
      applyMatchStyle(mark, true)
      mark.scrollIntoView({ block: 'center', inline: 'nearest' })
    } else {
      delete mark.dataset.luxcodexFindActive
      applyMatchStyle(mark, false)
    }
  })
}
