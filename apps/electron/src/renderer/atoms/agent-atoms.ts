...
import type { AgentSessionMeta, AgentEvent, AgentWorkspace, AgentPendingFile, RetryAttempt, YodaPermissionMode, PermissionRequest, AskUserRequest, ExitPlanModeRequest, ThinkingConfig, AgentEffort, SDKMessage, UnstagedChangesResult } from '@yoda/shared'
import { YODA_DEFAULT_PERMISSION_MODE } from '@yoda/shared'
...
export const agentSidePanelOpenAtom = atomWithStorage<boolean>('yoda-agent-sidepanel-open', true)

/** 侧面板宽度（全局共享，用户拖拽后持久化） */
export const agentSidePanelWidthAtom = atomWithStorage<number>('yoda-agent-sidepanel-width', 280)
...
/** 新会话默认权限模式 */
export const agentDefaultPermissionModeAtom = atom<YodaPermissionMode>(YODA_DEFAULT_PERMISSION_MODE)

/** Per-session 权限模式 Map — sessionId → YodaPermissionMode */
export const agentPermissionModeMapAtom = atom<Map<string, YodaPermissionMode>>(new Map())

/**
 * 按 sessionId 派生该 session 的持久化权限模式。
 * 返回 `undefined`（session 不存在或未设置）或具体的 YodaPermissionMode 字符串，
