...
import type { AgentStreamEvent, AgentStreamCompletePayload, AgentEvent, AgentStreamPayload, SDKAssistantMessage, SDKUserMessage, SDKSystemMessage, SDKContentBlock, SDKUserContentBlock, YodaEvent, AgentSessionMeta, ProviderType } from '@yoda/shared'
import { inferAgentSdkContextWindow, inferContextWindow } from '@yoda/shared'
...
export function useGlobalAgentListeners(): void {
  ...
  useEffect(() => {
    ...
    const activateExternalAgentRun = (event: Extract<YodaEvent, { type: 'external_run_started' }>): void => {
    ...
    const cleanupEvent = window.electronAPI.onAgentStreamEvent(
      (streamEvent: AgentStreamEvent) => {
        ...
        if (payload.kind === 'yoda_event' && payload.event.type === 'external_run_started') {
          activateExternalAgentRun(payload.event)
        }

        // 自动任务会话被用户接管（毕业）：向用户提示，后续定时运行将新建独立会话
        if (payload.kind === 'yoda_event' && payload.event.type === 'automation_graduated') {
        ...
        for (const event of legacyEvents) {
          ...
          if (event.type === 'task_backgrounded') {
            ...
            store.set(agentPermissionModeMapAtom, (prev: Map<string, import('@yoda/shared').YodaPermissionMode>) => {
