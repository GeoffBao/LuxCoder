 ...
 *
 * 载荷结构：
 *   application/x-yoda-file-panel → JSON string of FilePanelDragItem[]
 */

export const FILE_PANEL_DRAG_MIME = 'application/x-yoda-file-panel'

/**
 * 三点菜单「引用到 Agent」触发的事件名。
 * detail 携带 FilePanelDragItem[]；AgentView 监听后调用 RichTextInput.insertFileMentions。
 */
export const INSERT_FILE_MENTION_EVENT = 'yoda:insert-file-mention'
