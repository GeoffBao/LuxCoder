...
const SANDBOX = '/yoda/agent-workspaces/ws/session-1'
...
describe('resolveSessionCwd', () => {
  ...
  test('managed 状态（未绑定外部目录的 Project）优先于沙箱使用其托管 cwd', () => {
    const result = resolveSessionCwd({
      agentCwdMode: 'project',
      projectId: 'proj-1',
      resolveProjectCwd: projectResolver({ status: 'managed', cwd: '/yoda/projects/foo', displayPath: '/yoda/projects/foo' }),
      sandboxCwd: SANDBOX,
    })
    expect(result).toEqual({ cwd: '/yoda/projects/foo', source: 'project' })
  })
})
