...
export function buildAgentReplyCard(result: FormattedAgentResult, subtitle?: string): Record<string, unknown> {
  ...
  return {
    ...
    header: {
      title: { tag: 'plain_text', content: 'LuxCoder Agent' },
...
export function buildErrorCard(errorMessage: string): Record<string, unknown> {
  return {
    config: { wide_screen_mode: true },
    header: {
      title: { tag: 'plain_text', content: 'LuxCoder 错误' },
      template: 'red',
    },
    elements: [
...
export function buildSessionListCard(
  ...
  if (elements.length === 0) {
    elements.push({ tag: 'markdown', content: '暂无空间和会话' })
  }
  ...
  return {
    config: { wide_screen_mode: true },
    header: {
      title: { tag: 'plain_text', content: '空间与会话' },
      template: 'blue',
    },
...
/**
 * 构建空间切换结果卡片（/workspace 命令）
 *
 * 显示切换成功 + 该空间下最近会话列表，引导用户 /switch 或 /new。
 */
export function buildWorkspaceSwitchedCard(
  ...
  return {
    config: { wide_screen_mode: true },
    header: {
      title: { tag: 'plain_text', content: '切换空间' },
      template: 'green',
    },
    elements,
  }
...
export function buildWorkspaceListCard(
  ...
  return {
    config: { wide_screen_mode: true },
    header: {
      title: { tag: 'plain_text', content: '选择空间' },
      template: 'blue',
    },
    elements: [
      {
      ...
      {
        tag: 'note',
        elements: [{ tag: 'plain_text', content: '使用 /workspace <序号或名称> 切换空间' }],
      },
    ],
  }
}
...
export function buildHelpCard(): Record<string, unknown> {
  return {
    config: { wide_screen_mode: true },
    header: {
      title: { tag: 'plain_text', content: 'LuxCoder Bot 命令' },
      template: 'blue',
    },
    elements: [
      {
        ...
        content: [
          ...
          '`/now` — 查看当前状态（空间、会话、模型、MCP、Skills 等）',
          '`/list` (`/ls`) — 列出所有会话',
          '`/stop` (`/s`) — 停止当前 Agent',
          '`/switch` (`/sw`) `<序号>` — 切换到已有会话',
          '`/workspace` (`/ws`) `<名称>` — 设置空间',
