 ...
 *
 * 负责：
 * 1. 组织连接配置持久化（~/.yoda/org-settings.json）
...
function getOrgSettingsPath(): string {
  // 环境变量覆盖（测试/CI/可移植部署用）
  if (process.env.YODA_ORG_SETTINGS_PATH) {
    return process.env.YODA_ORG_SETTINGS_PATH
  }
  // 与 config-paths 一致：开发模式 .yoda-dev，正式版本 .yoda
  const dirName = process.env.YODA_DEV === '1' ? '.yoda-dev' : '.yoda'
