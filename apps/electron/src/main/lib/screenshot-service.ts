...
import { SCREENSHOT_LIMITS } from '@yoda/shared'
...
<meta charset="utf-8">
<meta http-equiv="Content-Security-Policy" content="default-src 'none'; img-src 'self' data: blob: yoda-file: https: http:; media-src 'self' data: blob: yoda-file: https: http:; font-src 'self' data: https: http:; style-src 'unsafe-inline'; script-src 'none'; connect-src 'none'; frame-src 'none'; object-src 'none'; base-uri 'none'; form-action 'none'">
<style>${safeCss}</style>
<style>
...
.yoda-screenshot-wrapper{padding:${SCREENSHOT_PADDING_TOP}px ${SCREENSHOT_PADDING_X}px;background:${bg};width:max-content;max-width:100%;margin:0 auto}
.yoda-screenshot-sheet{width:max-content;max-width:100%;margin:0 auto;background:${bg}}
.yoda-screenshot-sheet [contenteditable],
.yoda-screenshot-sheet [contenteditable="false"]{outline:none}
.yoda-screenshot-sheet .ProseMirror-selectednode,
.yoda-screenshot-sheet .selectedCell::after{display:none!important}
</style></head><body class="${safeThemeClass}">
<div class="yoda-screenshot-wrapper">
<main class="yoda-screenshot-sheet">${safeHtml}</main>
</div>
</body></html>`
}
...
async function screenshotCapture(htmlContent: string, width: number): Promise<Buffer> {
  const tmpPath = join(tmpdir(), `yoda-ss-${Date.now()}.html`)
  ...
  return withLock(async () => {
    try {
      ...
      const { canceled, filePath } = await dialog.showSaveDialog({
        title: '保存截图',
        defaultPath: join(homedir(), 'Desktop', `yoda-${ts}.png`),
