...
import { SCREENSHOT_LIMITS } from '@luxcoder/shared'
...
<meta charset="utf-8">
<meta http-equiv="Content-Security-Policy" content="default-src 'none'; img-src 'self' data: blob: luxcoder-file: https: http:; media-src 'self' data: blob: luxcoder-file: https: http:; font-src 'self' data: https: http:; style-src 'unsafe-inline'; script-src 'none'; connect-src 'none'; frame-src 'none'; object-src 'none'; base-uri 'none'; form-action 'none'">
<style>${safeCss}</style>
<style>
...
.luxcoder-screenshot-wrapper{padding:${SCREENSHOT_PADDING_TOP}px ${SCREENSHOT_PADDING_X}px;background:${bg};width:max-content;max-width:100%;margin:0 auto}
.luxcoder-screenshot-sheet{width:max-content;max-width:100%;margin:0 auto;background:${bg}}
.luxcoder-screenshot-sheet [contenteditable],
.luxcoder-screenshot-sheet [contenteditable="false"]{outline:none}
.luxcoder-screenshot-sheet .ProseMirror-selectednode,
.luxcoder-screenshot-sheet .selectedCell::after{display:none!important}
</style></head><body class="${safeThemeClass}">
<div class="luxcoder-screenshot-wrapper">
<main class="luxcoder-screenshot-sheet">${safeHtml}</main>
</div>
</body></html>`
}
...
async function screenshotCapture(htmlContent: string, width: number): Promise<Buffer> {
  const tmpPath = join(tmpdir(), `luxcoder-ss-${Date.now()}.html`)
  ...
  return withLock(async () => {
    try {
      ...
      const { canceled, filePath } = await dialog.showSaveDialog({
        title: '保存截图',
        defaultPath: join(homedir(), 'Desktop', `luxcoder-${ts}.png`),
