...
import {
  ...
  type YodaPermissionMode,
} from '@yoda/shared'

const PERMISSION_RANK: Record<YodaPermissionMode, number> = {
  plan: 0,
  bypassPermissions: 1,
}
...
export function resolveDelegationPermissionMode(
  parentMode: YodaPermissionMode | undefined,
  requestedMode: YodaPermissionMode | undefined,
  agentRuntime?: AgentRuntime,
): YodaPermissionMode {
  // Pi 子会话目前不支持 Plan 模式下的完整工具集，固定直接执行。
  if (agentRuntime === 'pi') return 'bypassPermissions'

  const parent = parentMode ?? YODA_DEFAULT_PERMISSION_MODE
...
export function buildRecoveredDelegationState(input: {
  ...
  fallbackPermissionMode?: YodaPermissionMode
  ...
  return {
    ...
    permissionMode: input.session.permissionMode ?? input.fallbackPermissionMode ?? YODA_DEFAULT_PERMISSION_MODE,
    status,
...
export function buildDelegationPrompt(input: {
  ...
  return `你是 Yoda 协作子 Agent。你由父 Agent 会话 ${input.parentSessionId} 委派创建，委派 ID 为 ${input.delegationId}。
