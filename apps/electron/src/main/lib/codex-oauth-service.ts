...
import type { CodexOAuthCredentials } from '@luxcoder/shared'
...
/**
 * Pi 的 ModelRuntime 只要求 CredentialStore 的结构契约。OAuth 凭据仍由 LuxCoder
 ...
 * LuxCoder 的代理 fetch 重新接回该流程；本 service 只负责与公开 ModelRuntime 交互。
 */
...
export async function loginCodexOAuth(callbacks?: CodexLoginCallbacks): Promise<CodexOAuthCredentials> {
  ...
  try {
    ...
    const credentials = await runtime.login('openai-codex', 'oauth', {
      signal: abort.signal,
      prompt: async (prompt) => {
        // Pi 先要求选择登录方式；LuxCoder v1 固定浏览器授权，回调服务会处理 code。
