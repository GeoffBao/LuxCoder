...
import type { ChangedFileEntry, UnstagedChangesResult, UntrackedFileEntry } from '@yoda/shared'
import { normalizePathForCompare } from '@yoda/shared'
import type { ChangeSource, ChangedFileStatus } from '@yoda/shared'
 ...
 *
 * 用于安全校验：worktree 常被放在主仓库之外（如 ~/yoda-dev/worktrees/xxx），
 ...
 */
export async function listWorktrees(repoPath: string): Promise<import('@yoda/shared').WorktreeInfo[]> {
  ...
  const worktrees: import('@yoda/shared').WorktreeInfo[] = []
...
export async function getWorktreeChanges(
...
): Promise<import('@yoda/shared').UnstagedChangesResult> {
  ...
  const allFiles: import('@yoda/shared').ChangedFileEntry[] = []
  const fileMap = new Map<string, import('@yoda/shared').ChangedFileEntry>()
  ...
  if (committedStatus) {
    for (const line of committedStatus.split('\n').filter(Boolean)) {
      ...
      let status: import('@yoda/shared').ChangedFileStatus
      let filePath: string
      ...
      const entry: import('@yoda/shared').ChangedFileEntry = {
        filePath,
        status,
        additions: stats.additions,
        deletions: stats.deletions,
  ...
  // 3. 新文件（未追踪）
  const untrackedFiles: import('@yoda/shared').UntrackedFileEntry[] = []
  const untrackedOutput = await runGitCommand(['ls-files', '--others', '--exclude-standard'], gitRoot)
  if (untrackedOutput) {
    for (const rel of untrackedOutput.split('\n').filter(Boolean)) {
      if (!fileMap.has(rel)) {
