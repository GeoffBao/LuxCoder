/**
 * LuxCoder Git / PR 归因标识
 *
 * 目标：当 Agent 代用户创建 commit / PR 时，附带可搜索、可关闭的 LuxCoder 标识，
...
/** 开源仓库完整地址 */
export const LUXCODER_GITHUB_URL = 'https://github.com/GeoffBao/LuxCoder'

/** Commit trailer（标准 git trailer，不进入 GitHub co-author 列表） */
export const LUXCODER_COMMIT_TRAILER = 'Made-with: LuxCoder'

/**
 * PR / MR 描述底部标识。
 * 含开源仓库完整链接，便于推广与引流。
 */
export const LUXCODER_PR_ATTRIBUTION =
  `Made with [LuxCoder](${LUXCODER_GITHUB_URL})`
...
export function buildClaudeSdkAttribution(enabled: boolean): { commit: string; pr: string } {
  ...
  return {
    commit: LUXCODER_COMMIT_TRAILER,
    pr: LUXCODER_PR_ATTRIBUTION,
  }
}

/**
 * 将 LuxCoder attribution 合并进 Claude session 的 settings 对象。
...
用户已在 LuxCoder 设置中关闭「Git/PR 标识」。当你创建 git commit 或 Pull Request / Merge Request 时：
- **不要**添加任何 LuxCoder 相关归因（包括 \`${LUXCODER_COMMIT_TRAILER}\`、\`${LUXCODER_PR_ATTRIBUTION}\`、\`Co-Authored-By: LuxCoder\`、\`Generated with LuxCoder\` 等）
- 也不要主动建议用户打开该标识；仅当用户明确要求时再添加`
  }

  return `## Git / PR 标识（推广归因，默认开启）

当**你**通过 Bash 等工具代用户创建 git commit 或 Pull Request / Merge Request 时，必须附加 LuxCoder 标识（用户当次明确说「不要带标识 / 不要 Made-with」除外）。

### Commit
在 commit message **末尾**追加标准 git trailer（与正文之间空一行）：

\`\`\`text
${LUXCODER_COMMIT_TRAILER}
\`\`\`

规则：
- **只用** \`${LUXCODER_COMMIT_TRAILER}\` trailer，**不要**使用 \`Co-Authored-By: ...\`（避免假冒作者、污染 GitHub contributors）
- 若 message 已含 \`Made-with: LuxCoder\`（大小写不敏感），不要重复
- 推荐：\`git commit --trailer "${LUXCODER_COMMIT_TRAILER}" -m "subject"\`；或多段 \`-m\`，确保 trailer 单独处于最后一段且格式为 \`Key: Value\`
...
\`\`\`text
${LUXCODER_PR_ATTRIBUTION}
\`\`\`

- 开源仓库：\`${LUXCODER_GITHUB_URL}\`（不要写成其他错误地址）
- 若 body 已含 \`github.com/GeoffBao/LuxCoder\` 的 Made with / Generated with 行，不要重复
