...
export const MYYODA_COMMIT_TRAILER = 'Made-with: MyYoda'
...
- **不要**添加任何 MyYoda 相关归因（包括 \`${MYYODA_COMMIT_TRAILER}\`、\`${MYYODA_PR_ATTRIBUTION}\`、\`Co-Authored-By: MyYoda\`、\`Generated with MyYoda\` 等）
- 也不要主动建议用户打开该标识；仅当用户明确要求时再添加`
  }

  return `## Git / PR 标识（推广归因，默认开启）

当**你**通过 Bash 等工具代用户创建 git commit 或 Pull Request / Merge Request 时，必须附加 MyYoda 标识（用户当次明确说「不要带标识 / 不要 Made-with」除外）。

### Commit
在 commit message **末尾**追加标准 git trailer（与正文之间空一行）：

\`\`\`text
${MYYODA_COMMIT_TRAILER}
\`\`\`

规则：
- **只用** \`${MYYODA_COMMIT_TRAILER}\` trailer，**不要**使用 \`Co-Authored-By: ...\`（避免假冒作者、污染 GitHub contributors）
- 若 message 已含 \`Made-with: MyYoda\`（大小写不敏感），不要重复
- 推荐：\`git commit --trailer "${MYYODA_COMMIT_TRAILER}" -m "subject"\`；或多段 \`-m\`，确保 trailer 单独处于最后一段且格式为 \`Key: Value\`
