 ...
 * Yoda Git / PR 归因标识
 *
 * 目标：当 Agent 代用户创建 commit / PR 时，附带可搜索、可关闭的 Yoda 标识，
...
/** 开源仓库完整地址 */
export const YODA_GITHUB_URL = 'https://github.com/biajidong/Yoda'

/** Commit trailer（标准 git trailer，不进入 GitHub co-author 列表） */
export const YODA_COMMIT_TRAILER = 'Made-with: Yoda'

/**
 * PR / MR 描述底部标识。
 * 含开源仓库完整链接，便于推广与引流。
 */
export const YODA_PR_ATTRIBUTION =
  `Made with [Yoda](${YODA_GITHUB_URL})`
...
export function buildClaudeSdkAttribution(enabled: boolean): { commit: string; pr: string } {
  ...
  return {
    commit: YODA_COMMIT_TRAILER,
    pr: YODA_PR_ATTRIBUTION,
  }
}

/**
 * 将 Yoda attribution 合并进 Claude session 的 settings 对象。
...
用户已在 Yoda 设置中关闭「Git/PR 标识」。当你创建 git commit 或 Pull Request / Merge Request 时：
- **不要**添加任何 Yoda 相关归因（包括 \`${YODA_COMMIT_TRAILER}\`、\`${YODA_PR_ATTRIBUTION}\`、\`Co-Authored-By: Yoda\`、\`Generated with Yoda\` 等）
- 也不要主动建议用户打开该标识；仅当用户明确要求时再添加`
  }

  return `## Git / PR 标识（推广归因，默认开启）

当**你**通过 Bash 等工具代用户创建 git commit 或 Pull Request / Merge Request 时，必须附加 Yoda 标识（用户当次明确说「不要带标识 / 不要 Made-with」除外）。
...
规则：
- **只用** \`${YODA_COMMIT_TRAILER}\` trailer，**不要**使用 \`Co-Authored-By: ...\`（避免假冒作者、污染 GitHub contributors）
- 若 message 已含 \`Made-with: Yoda\`（大小写不敏感），不要重复
- 推荐：\`git commit --trailer "${YODA_COMMIT_TRAILER}" -m "subject"\`；或多段 \`-m\`，确保 trailer 单独处于最后一段且格式为 \`Key: Value\`
...
- 开源仓库：\`${YODA_GITHUB_URL}\`（不要写成其他错误地址）
- 若 body 已含 \`github.com/biajidong/Yoda\` 的 Made with / Generated with 行，不要重复
