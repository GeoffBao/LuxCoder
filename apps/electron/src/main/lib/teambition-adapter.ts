...
import type { McpServerEntry } from '@luxagents/shared'
import {
...
export class McpTeambitionAdapter implements TeambitionAdapter {
  ...
  private async connect(): Promise<Client> {
    ...
    const client = new Client({ name: 'luxagents-work-mode', version: '1.0.0' })
  ...
  async syncProgress(taskId: string, progress: number, idempotencyKey: string): Promise<void> {
    ...
    await this.postComment(taskId, `[LuxAgents:${idempotencyKey}] 任务进度 ${progress}%`)
  }
