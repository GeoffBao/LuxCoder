...
function writeAgentSessionJsonl(sessionId: string, rows: string[]): void {
  const dir = join(tempHome, '.luxcodex', 'agent-sessions')
...
function writeSdkSessionJsonl(sdkSessionId: string, rows: string[]): void {
  const dir = join(tempHome, '.luxcodex', 'sdk-config', 'projects', 'test-project')
  mkdirSync(dir, { recursive: true })
...
function writeAgentSessionsIndex(sessions: Array<{
  ...
  const dir = join(tempHome, '.luxcodex')
  mkdirSync(dir, { recursive: true })
...
beforeAll(async () => {
  tempHome = mkdtempSync(join(os.tmpdir(), 'luxcodex-agent-session-manager-'))
