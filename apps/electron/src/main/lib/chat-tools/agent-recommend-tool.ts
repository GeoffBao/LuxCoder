...
import type { ToolCall, ToolResult, ToolDefinition } from '@yoda/core'
import type { ChatToolMeta } from '@yoda/shared'
...
你有能力推荐用户切换到 Project 模式。**请积极主动地识别适合 Agent 模式的任务**，在回复用户之前先调用 suggest_agent_mode 工具。

## Agent 模式的能力

Agent 模式拥有以下 Chat 模式不具备的能力：
- **文件读写**：直接浏览、创建、编辑用户本地文件和工作区代码
- **命令行操作**：执行 shell 命令、运行测试、安装依赖、构建工程
...
3. **文件操作意图**：阅读/创建/修改代码、文档、配置文件，生成工作区结构等
...
- "分析一下这个工作区的代码" → 推荐（文件读取 + 代码分析）
- "帮我整理一份报告" → 推荐（内容创作 + 结构化输出）
- "搭建一个新工作区" → 推荐（文件创建 + 命令行 + 多步骤）
