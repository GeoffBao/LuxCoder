...
export function previewVoiceDictationText(
  ...
  if (!text || !shouldWriteToLuxCoderInput(settings)) return
...
export async function commitVoiceDictationText(
  ...
  if ((hasActivePreview || shouldWriteToLuxCoderInput(settings)) && sendTextEvent(VOICE_DICTATION_IPC_CHANNELS.INSERT_TEXT, {
