...
import type { OfficePreviewResult } from '@yoda/shared'
...
function getPreviewTmpDir(): string {
  const dir = join(tmpdir(), 'yoda-preview')
  if (!existsSync(dir)) {
...
/** 为内联 HTML 预览注册文件所在目录的 yoda-file:// URL（相对路径资源自动解析） */
export async function prepareHtmlPreview(filePath: string, basePaths?: string[]): Promise<{ resolvedPath: string; tmpUrl: string } | null> {
  ...
  try {
    ...
    // 目录 URL 形如 yoda-file://{token}，拼接文件名后 iframe 可直接加载；
