...
function buildWebTools(sdk: PiSdk): ToolDefinition[] {
  return [
    sdk.defineTool({
      name: 'WebSearch',
      label: '搜索网页',
      description: 'Search the web for up-to-date information through LuxCoder Tavily integration. Use for current events, recent data, facts that may be stale, or when the user explicitly asks to search.',
    ...
    sdk.defineTool({
      name: 'WebFetch',
      label: '抓取网页',
      description: 'Fetch and extract readable Markdown content from a URL through LuxCoder Tavily integration. Use after WebSearch or when the user gives a URL and asks to inspect page content.',
...
function summarizeAutomation(a: import('@luxcoder/shared').Automation, includeHistory: boolean): AutomationSummary {
  return {
    id: a.id,
    name: a.name,
    active: a.active,
...
function buildAutomationTools(sdk: PiSdk, ctx: PiBuiltinToolsContext): ToolDefinition[] {
  return [
    sdk.defineTool({
      ...
      label: '列出定时任务',
      description: '列出 LuxCoder 持久化定时任务。用于查看已有长期反复任务、判断是否需要新建任务、检查运行状态和最近失败情况。',
      parameters: Type.Object({
    ...
    sdk.defineTool({
      ...
      label: '查看定时任务',
      description: '读取单个 LuxCoder 定时任务详情和运行记录。定时任务自动执行中可以省略 id 来读取当前任务，用于自检和自迭代。',
      parameters: Type.Object({
    ...
    sdk.defineTool({
      ...
      label: '创建定时任务',
      description: '创建 LuxCoder 持久化定时任务。适合无人值守、有稳定价值的场景。纯提醒/闹钟、需要用户实时参与判断、或现在就该做完即终结的事不要创建。',
      parameters: Type.Object({
    ...
    sdk.defineTool({
      ...
      label: '修改定时任务',
      description: '修改 LuxCoder 定时任务，包括名称、执行提示词、频率和启用状态。定时任务自动执行中可以省略 id 来修改当前任务。',
      parameters: Type.Object({
    ...
    sdk.defineTool({
      ...
      label: '删除定时任务',
      description: '删除 LuxCoder 定时任务。只在用户明确要求删除，或任务已经长期无价值且用户确认后使用。',
      parameters: Type.Object({
    ...
    sdk.defineTool({
      ...
      label: '立即运行定时任务',
      description: '立即运行 LuxCoder 定时任务。用于用户要求马上验证，或修改任务后需要试跑一次。',
      parameters: Type.Object({
...
function buildPlanningTools(sdk: PiSdk, ctx: PiBuiltinToolsContext): ToolDefinition[] {
  ...
  return [
    sdk.defineTool({
      ...
      description: '列出 LuxCoder Todo（包含用户明确连接的系统提醒事项投影）。返回项的 nativeOrigin 表示编辑会写回系统；对该类项单项编辑/完成先征得用户确认，批量修改和删除必须明确确认。仅 Pi Agent 可用。',
      parameters: Type.Object({
    ...
    sdk.defineTool({
      ...
      description: '创建 LuxCoder 本地 Todo。调用前必须先用 list_todos(status=open) 检查重复，并用 list_groups({ scope: todo }) 查询并优先复用 Todo 分组；用户明确提出待办，或可合理确定下一步时使用。未传 dueAt 时默认当天结束前；仅 Pi Agent 可用。',
    ...
    sdk.defineTool({
      ...
      description: '列出 LuxCoder 日程（包含用户明确连接的系统日历投影）。nativeOrigin 表示编辑会写回系统；Agent 修改前必须先取得用户确认。仅 Pi Agent 可用。',
      parameters: Type.Object({
    ...
    sdk.defineTool({
      ...
      description: '创建 LuxCoder 本地日程。分组必须来自 list_groups({ scope: calendar })；用户明确提供时间安排时使用。仅 Pi Agent 可用。',
