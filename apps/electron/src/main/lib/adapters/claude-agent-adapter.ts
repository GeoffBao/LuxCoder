...
export interface ClaudeAgentQueryOptions extends AgentQueryInput {
  ...
  maxTurns?: number
  /** SDK 权限模式（LuxCodex 当前三种模式直接映射 SDK 原生模式） */
  sdkPermissionMode: LuxCodexPermissionMode
  /** 是否跳过权限检查 */
...
export function mapSDKErrorToTypedError(
  ...
  const isInvalidChannelOrModel = /请检查是否选择了正确的 LuxCodex 供应渠道和模型/.test(mapped.message)
