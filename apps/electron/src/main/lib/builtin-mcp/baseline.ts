 ...
 * Yoda 每次发布即覆盖升级，无需运行时同步到用户目录。
 */

import type { BuiltinMcpCategory, McpToolSummary } from '@yoda/shared'
import manifest from './default-mcp.json' with { type: 'json' }

export interface BuiltinMcpDefinition {
  /** 设置 / 凭据键，历史值，向后兼容（如 'yoda-cloud'、'nano-banana'） */
  id: string
  ...
  /** 是否提供开关（基础设施型如 yoda-cloud 置 false） */
  toggleable: boolean
