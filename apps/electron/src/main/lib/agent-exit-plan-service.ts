...
export type ExitPlanPermissionResult = {
  ...
  /** 用户选择的目标权限模式 */
  targetMode?: LuxAgentsPermissionMode
} | {
  behavior: 'deny'
  message: string
}
...
export interface ExitPlanModeCallbacks {
  /** 切换权限模式 */
  onPermissionModeChange: (mode: LuxAgentsPermissionMode) => void
}
...
export class AgentExitPlanService {
   ...
   */
  respondToExitPlanMode(response: ExitPlanModeResponse): { sessionId: string; targetMode: LuxAgentsPermissionMode | null } | null {
