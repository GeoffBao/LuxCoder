...
if (!app.requestSingleInstanceLock()) {
  console.warn(
    '[启动] 已有 LuxCodex 进程持有单实例锁，本次启动将退出。\n' +
      '  如果窗口未出现，可能旧进程已卡死。请运行 `killall LuxCodex` 后重试。',
  )
  app.quit()
} else {
  // 主流程：正常启动（单实例锁已获取）
...
function registerProtocolsAndHandlers(): void {
  ...
  // 用于内联预览本地文件（renderer 用 iframe 加载 luxcodex-file:// 资源）
...
import { setAppVersion } from '@luxcodex/core'
...
async function bootstrap(): Promise<void> {
  ...
  // 初始化 LuxCodex 版本号（供 User-Agent 等全局标识使用）
  setAppVersion(app.getVersion())

  // 注册自定义协议 luxcodex-file:// 用于内联预览本地文件。
  ...
  // 种子内置 Agent 专家包到 ~/.luxcodex/experts/
  ...
  try {
    ...
    dialog.showErrorBox(
      ...
      `部分功能可能不可用：\n\n${message}\n\n` +
        ...
        `常见原因与排查：\n` +
        `1. 旧版 LuxCodex 进程未退出（终端运行 killall LuxCodex 后重试）\n` +
