...
import { IPC_CHANNELS, CHANNEL_IPC_CHANNELS, CHAT_IPC_CHANNELS, AGENT_IPC_CHANNELS, ENVIRONMENT_IPC_CHANNELS, INSTALLER_IPC_CHANNELS, PROXY_IPC_CHANNELS, GITHUB_RELEASE_IPC_CHANNELS, SYSTEM_PROMPT_IPC_CHANNELS, CHAT_TOOL_IPC_CHANNELS, FEISHU_IPC_CHANNELS, DINGTALK_IPC_CHANNELS, WECHAT_IPC_CHANNELS, AUTOMATION_IPC_CHANNELS, EXPERT_IPC_CHANNELS, AGENT_THINKING_LEVELS, isYodaPermissionMode, normalizePathForCompare, PLANNING_IPC_CHANNELS, RELEASE_NOTES_IPC_CHANNELS, type PlanningWorkspaceScope } from '@yoda/shared'
...
import { serializeCodexCredentials, serializeClaudeOAuthCredentials, serializeXaiCredentials } from '@yoda/shared'
import type { CodexOAuthDeviceCode, CodexOAuthLoginMethod, XaiOAuthDeviceCode } from '@yoda/shared'
 ...
 *
 * worktree 常被放在主仓库之外（如 ~/yoda-dev/worktrees/xxx），其路径不在任何
...
async function ensurePathAllowedWithWorktree(filePath: string, options?: FileAccessOptions): Promise<boolean> {
  ...
  if (mainRepo) {
    ...
    for (const workspaceSlug of getWorkspaceSlugsForAccess(options)) {
      let repos: import('@yoda/shared').WorkspaceWorktreeRepo[]
      try {
 ...
 */
const defaultAppCache = new Map<string, import('@yoda/shared').DefaultAppInfo>()
...
async function getMacAppIconViaSips(appPath: string): Promise<string> {
  ...
  const tmp = mkdtempSync(join(tmpdir(), 'yoda-icon-'))
...
): Promise<import('@yoda/shared').DefaultAppInfo | null> {
...
if let appUrl = NSWorkspace.shared.urlForApplication(toOpen: url) {
  ...
  const info: import('@yoda/shared').DefaultAppInfo = { name: appName, appPath, iconDataUrl }
...
export function registerIpcHandlers(): void {
  ...
  ipcMain.handle(
    ...
    async (): Promise<import('@yoda/shared').EditorApp[]> => {
  ...
  ipcMain.handle(
    ...
    async (_, filePath: string, access?: FileAccessOptions | string[]): Promise<import('@yoda/shared').DefaultAppInfo | null> => {
  ...
  ipcMain.handle(
    ...
    async (_, channelId: string): Promise<import('@yoda/shared').ChannelPlanQuotaResult> => {
  ...
  ipcMain.handle(
    ...
    async (event, requestedMethod?: CodexOAuthLoginMethod): Promise<import('@yoda/shared').CodexOAuthLoginResult> => {
  ...
  ipcMain.handle(
    ...
    async (): Promise<import('@yoda/shared').ClaudeOAuthPrepareResult> => {
      try {
  ...
  ipcMain.handle(
    ...
    async (_event, code: string): Promise<import('@yoda/shared').ClaudeOAuthLoginResult> => {
      try {
  ...
  ipcMain.handle(
    ...
    async (event): Promise<import('@yoda/shared').XaiOAuthLoginResult> => {
      try {
  ...
  ipcMain.handle(
    ...
    async (_, name: string, entry: import('@yoda/shared').McpServerEntry): Promise<{ success: boolean; message: string }> => {
  ...
  ipcMain.handle(
    ...
    async (event, input: import('@yoda/shared').AgentQueueMessageInput): Promise<string> => {
    ...
    async (event, response: PermissionResponse): Promise<void> => {
      ...
      if (sessionId) {
        event.sender.send(AGENT_IPC_CHANNELS.STREAM_EVENT, {
          sessionId,
          payload: { kind: 'yoda_event', event: { type: 'permission_resolved', requestId, behavior } },
        })
      }
    }
  )
  ...
  ipcMain.handle(
    ...
    async (_, sessionId: string, mode: YodaPermissionMode): Promise<void> => {
      if (!isYodaPermissionMode(mode)) {
  ...
  ipcMain.handle(
    ...
    async (event, response: AskUserResponse): Promise<void> => {
      ...
      if (sessionId) {
        event.sender.send(AGENT_IPC_CHANNELS.STREAM_EVENT, {
          sessionId,
          payload: { kind: 'yoda_event', event: { type: 'ask_user_resolved', requestId } },
        })
      }
    }
  )
  ...
  ipcMain.handle(
    ...
    async (event, response: ExitPlanModeResponse): Promise<void> => {
      ...
      if (result) {
        ...
        event.sender.send(AGENT_IPC_CHANNELS.STREAM_EVENT, {
          sessionId,
          payload: { kind: 'yoda_event', event: { type: 'exit_plan_mode_resolved', requestId: response.requestId } },
        })
        ...
        if (targetMode) {
          ...
          event.sender.send(AGENT_IPC_CHANNELS.STREAM_EVENT, {
            sessionId,
            payload: { kind: 'yoda_event', event: { type: 'permission_mode_changed', mode: targetMode } },
          })
  ...
  ipcMain.handle(
    ...
    async (): Promise<import('@yoda/shared').PendingRequestsSnapshot> => {
      return {
  ...
  ipcMain.handle(
    ...
    async (_, workspaceSlug: string, repo: import('@yoda/shared').WorkspaceWorktreeRepo) => {
  ...
  ipcMain.handle(
    ...
    async (_, filename: string, content: string): Promise<string> => {
      ...
      const tmpDir = join(tmpdir(), 'yoda-preview')
  ...
  ipcMain.handle(
    'file:office-to-html',
    async (_, filePath: string, access?: FileAccessOptions | string[]): Promise<import('@yoda/shared').OfficePreviewResult | null> => {
  ...
  ipcMain.handle(
    ...
    async (_, input: import('@yoda/shared').FeishuBotConfigInput) => {
  ...
  ipcMain.handle(
    ...
    async (_, input: import('@yoda/shared').DingTalkBotConfigInput) => {
  ...
  ipcMain.handle('migration:cancelImport', async (_, tempDir: string) => {
    if (tempDir && existsSync(tempDir) && tempDir.includes('yoda-import-')) {
  ...
  ipcMain.handle(
    ...
    async (event, input?: VoiceDictationToggleInput): Promise<void> => {
      ...
      const sourceInputId = typeof input?.sourceInputId === 'string' && input.sourceInputId.length > 0 && input.sourceInputId.length <= 512
        ? input.sourceInputId
        : undefined
      toggleVoiceDictationWindow({ targetIsYoda: !!sourceWindow, sourceInputId })
    }
  )
  ...
  ipcMain.handle('migration:openFileDialog', async () => {
    ...
    const result = await dialog.showOpenDialog({
      title: '选择迁移文件',
      filters: [
        { name: 'Yoda 迁移文件', extensions: ['yoda-backup', 'yoda-share'] },
        { name: '所有文件', extensions: ['*'] },
      ],
      properties: ['openFile'],
    })
  ...
  ipcMain.handle('migration:saveFileDialog', async (_, mode: string) => {
    const { dialog } = await import('electron')
    const ext = mode === 'personal' ? 'yoda-backup' : 'yoda-share'
    const defaultName = `yoda-migration-${new Date().toISOString().slice(0, 10)}.${ext}`
    const result = await dialog.showSaveDialog({
      title: '保存迁移文件',
      defaultPath: defaultName,
      filters: [
        { name: mode === 'personal' ? 'Yoda 个人备份' : 'Yoda 分享包', extensions: [ext] },
      ],
    })
    return result.canceled ? null : result.filePath
  })
  ...
  // 否则 NaN / -Infinity / 越界值会污染 ~/.yoda/automations.json，无法回滚。
