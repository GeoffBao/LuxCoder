...
Yoda 是基于 Proma 演化的企业级 AI R&D 工作台，采用 Electron 桌面应用架构。UI 顶层模式：Chat / Code（Agent） / Work（Kanban）。
...
**包命名规范**：`@yoda/*` 作用域（`@yoda/core`、`@yoda/shared`、`@yoda/ui`、`@yoda/electron`）
...
1. **类型 & 常量**：`@yoda/shared` 定义 IPC 通道名称常量和请求/响应类型
...
| `config-paths.ts` | 配置路径管理：`~/.yoda/` 目录结构 |
...
- monorepo 内各包（`@yoda/shared` / `core` / `ui` / `electron`）各自有独立 `version`，互不自动同步：
...
**为什么**：`seedDefaultSkills()` 与 `upgradeDefaultSkillsInWorkspaces()` 通过 semver 比较决定是否将 bundle 中的 Skill 同步到老用户的 `~/.yoda/default-skills/` 与各工作区。**version 不变 = 老用户拿不到新内容**。
...
- **事件转换**：`convertSDKMessage()`（`@yoda/shared`）将 SDK 原始消息转为统一的 `AgentEvent` 类型
...
- `0.2.113`:
  ...
  - 详见上方"打包配置注意事项"段落
- `0.2.120`: `query()` 省略 `settingSources` 时默认加载所有来源（Yoda 已显式传 `['user', 'project']`，不受影响）
...
- Task `llmConnection` maps to Yoda `channelId` (Channel acts as the connection)
- Dev config/storage lives under `~/.yoda-dev/`; packaged builds use `~/.yoda/`
